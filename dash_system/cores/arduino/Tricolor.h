/*
  Tricolor.h - Class controls the RGB LED..
...
{
public:
    Tricolor(uint32_t r, uint32_t g, uint32_t b, bool invert=false);
    void enable(bool en);
    void off();
    void on(uint8_t r, uint8_t g, uint8_t b);
    void on(ColorRGB color);
    void on(uint32_t rgb);
...
protected:
    ...
    uint8_t g;
    uint8_t b;
    void set(uint32_t pin, uint8_t value);
};
